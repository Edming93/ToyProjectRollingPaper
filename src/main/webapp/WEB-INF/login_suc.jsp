...
<head>
  ...
  <style>
    @font-face {
      ...
      src:
        url('https://cdn.jsdelivr.net/gh/projectnoonnu/noonfonts_2110@1.0/KOTRAHOPE.woff2') format('woff2');
      font-weight: normal;
      font-style: normal;
    }
...
<body>
    ...
    <c:forEach var="item" items="${content}">
      <div class="modal">
        <div class="modal-inner1">
          <h1>Quiz를 풀어주세요!</h1>
          <h2>문제) <span>${item.quiz}</span></h2>
          <div id="quiz_area">
          	<input type="hidden" class="quiz_id" value="${item.id}">
            <input type="text" id="quiz" name="quiz" class="quiz_answer" required />
  ...
  <script type="text/javascript">
    ...
    let quiz_answer = document.querySelectorAll(".quiz_answer");
    ...
    for (let index = 0; index < document.getElementsByClassName("btn").length; index++) {
      document.getElementsByClassName("btn")[index].addEventListener("click", function (e) {
        ...
        let quiz_id = document.getElementsByClassName("quiz_id")[index];
        console.log(quiz_id);
        
         fetch("${pageContext.request.contextPath}/fetch",{
          method : "POST",
          headers : {"Content-Type" : "application/json"},
          body : JSON.stringify({id: quiz_id.value})
        }).then(response => response.json())
        .then(data => {
        	console.dir(data);
        	console.log(data.answer);
        	console.log(quiz_answer[index].value);
        	
          if(quiz_answer[index].value == data.answer){
